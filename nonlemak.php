<?php
 ...
 $limit = (isset($_GET['limit'])) ? $_GET['limit'] :4;
  ...
  $inputbox=$_POST['inputbox'];
   ...
   $query ="SELECT * FROM barang where CONCAT(judul,harga,keterangan,jenispaket) like '%".$inputbox."%' AND harga BETWEEN 400 AND 7000  ";
...
$results=$paginator->getData($limit,$page);
    ...
    $query ="SELECT * FROM barang where harga BETWEEN 0 AND 100";
...
<?php include'header.php';?>
   <div id="bodyholder">
        ...
        <a href="nonlemak.php" class="order-customization-link button button-red <?php if(isset($_POST['inputbox'])){ echo"";}else{echo 'hidden';}?>">Kembali</a>
            <h1 class="item-title blue"><?php if(isset($_POST['inputbox'])){ echo"Pencarian Kategori Non Lemak";}else{echo'Kategori Non Lemak';}?></h1>
 ...
 <?php
        ...
        $nonlemak=$results->data[$i]; 
        ?>
                               
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 centered menu-hover">
                                       <a href="lihatbarang.php?id=<?php echo $nonlemak['id'];?>" class="active" >
                                        <img width="250" height="250" class="img-circle" src="<?php echo "imgevent/".$nonlemak['gambar'];?>" class="items" alt="Gambar">
                                           </a>
                                        <h2 class="item-name"><?php echo $nonlemak['judul'];?></h2>
                                        <?php if($_SESSION['uid']){echo'<form method="post" action="?action=rating">';}else{echo"";}?>
                  <input type="hidden" name="idrating" value="<?php echo $nonlemak['id']; ?>">
                <input type="hidden" name="namarating" value="<?php echo $nonlemak['judul']; ?>">
                <input type="hidden" name="gambarrating" value="<?php echo $nonlemak['gambar']; ?>">
                  <button onclick="document.getElementById('login').style.display='<?php if($_SESSION['uid']){echo'none';}else{echo"block";}?>'"> 
                <input  name="ratingdepan" id="input-21e" value="<?php echo $nonlemak["rating"]; ?>"   class="rating" data-min=0 data-max=5 data-step=0.1 data-size="xs">
                </button>
                 <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
                                            <p class="calories"><img src="img/icon-budget.png" style="">Rp <span><?php echo $nonlemak['harga'];?></span>,00</p>
                                            <p style="text-transform: capitalize;" class="calories"><span><?php $r=$nonlemak['keterangan']; $a=explode(",", $r); echo $a['0']; ?></span></p>
                                                                                <div class="button-wrapper">
                                            <a href="lihatbarang.php?id=<?php echo $nonlemak['id'];?>" class="button-blue view-menu-item" title="">VIEW</a>
          ...
          <button class="order-customization-link button button-red <?php if($_SESSION['uid']){echo'';}else{echo'hidden';}?>" type="submit">Add to Cart</button>
            ...
            <input type="hidden" name="foto" value="<?php echo $nonlemak["gambar"]; ?>" />
            <input type="hidden" name="nama" value="<?php echo $nonlemak["judul"]; ?>" />
            <input type="hidden" name="id" value="<?php echo $nonlemak["id"]; ?>" />
            <input type="hidden" name="harga" value="<?php echo $nonlemak["harga"]; ?>" />
              <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
