 ...
 $limit = (isset($_GET['limit'])) ? $_GET['limit'] :4;
  ...
  $inputbox=$_POST['inputbox'];
   ...
   $query ="SELECT * FROM barang where CONCAT(judul,harga,keterangan,jenispaket) like '%".$inputbox."%' AND  jenispaket='nasitumpeng'";
...
$results=$paginator->getData($limit,$page);
    ...
    $query ="SELECT * FROM barang where jenispaket like '%nasitumpeng%'";
...
<?php include'header.php';?>
   <div id="bodyholder">
        ...
        <a href="nasitumpeng.php" class="order-customization-link button button-red <?php if(isset($_POST['inputbox'])){ echo"";}else{echo 'hidden';}?>">Kembali</a>
            <h1 class="item-title blue"><?php if(isset($_POST['inputbox'])){ echo"Pencarian Nasi Tumpeng";}else{echo'Nasi Tumpeng';}?></h1>
 ...
 <?php
        ...
        ?>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 centered menu-hover">
                                       <a href="lihatbarang.php?id=<?php echo $nasitumpeng['id'];?>" class="active" >
                  ...
                  <input type="hidden" name="idrating" value="<?php echo $nasitumpeng['id']; ?>">
                <input type="hidden" name="namarating" value="<?php echo $nasitumpeng['judul']; ?>">
                <input type="hidden" name="gambarrating" value="<?php echo $nasitumpeng['gambar']; ?>">
                  <button onclick="document.getElementById('login').style.display='<?php if($_SESSION['uid']){echo'none';}else{echo"block";}?>'"> 
                <input  name="ratingdepan" id="input-21e" value="<?php echo $nasitumpeng["rating"]; ?>"   class="rating" data-min=0 data-max=5 data-step=0.1 data-size="xs">
                </button>
                 <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
                                            ...
                                            <a href="lihatbarang.php?id=<?php echo $nasitumpeng['id'];?>" class="button-blue view-menu-item" title="">VIEW</a>
          ...
          <button class="order-customization-link button button-red <?php if($_SESSION['uid']){echo'';}else{echo'hidden';}?>" type="submit">Add to Cart</button>
            ...
            <input type="hidden" name="foto" value="<?php echo $nasitumpeng["gambar"]; ?>" />
            <input type="hidden" name="nama" value="<?php echo $nasitumpeng["judul"]; ?>" />
            <input type="hidden" name="id" value="<?php echo $nasitumpeng["id"]; ?>" />
            <input type="hidden" name="harga" value="<?php echo $nasitumpeng["harga"]; ?>" />
              <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
