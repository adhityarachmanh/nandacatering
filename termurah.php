<?php
 ...
 $limit = (isset($_GET['limit'])) ? $_GET['limit'] :4;
  ...
  $inputbox=$_POST['inputbox'];
   ...
   $query ="SELECT * FROM barang where CONCAT(judul,harga,keterangan,jenispaket) like '%".$inputbox."%' AND harga<50";
...
$results=$paginator->getData($limit,$page);
    ...
    $query ="SELECT * FROM barang where harga<50";
...
<?php include'header.php';?>
   <div id="bodyholder">
        ...
        <a href="termurah.php" class="order-customization-link button button-red <?php if(isset($_POST['inputbox'])){ echo"";}else{echo 'hidden';}?>">Kembali</a>
            <h1 class="item-title blue"><?php if(isset($_POST['inputbox'])){ echo"Pencarian Kategori Termurah";}else{echo'Kategori Termurah';}?></h1>
 ...
 <?php
        ...
        $termurah=$results->data[$i]; 
        ?>
                               
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 centered menu-hover">
                                       <a href="lihatbarang.php?id=<?php echo $termurah['id'];?>" class="active" >
                                        <img width="250" height="250" class="img-circle" src="<?php echo "imgevent/".$termurah['gambar'];?>" class="items" alt="Gambar">
                                           </a>
                                        <h2 class="item-name"><?php echo $termurah['judul'];?></h2>
                                        <?php if($_SESSION['uid']){echo'<form method="post" action="?action=rating">';}else{echo"";}?>
                  <input type="hidden" name="idrating" value="<?php echo $termurah['id']; ?>">
                <input type="hidden" name="namarating" value="<?php echo $termurah['judul']; ?>">
                <input type="hidden" name="gambarrating" value="<?php echo $termurah['gambar']; ?>">
                  <button onclick="document.getElementById('login').style.display='<?php if($_SESSION['uid']){echo'none';}else{echo"block";}?>'"> 
                <input  name="ratingdepan" id="input-21e" value="<?php echo $termurah["rating"]; ?>"   class="rating" data-min=0 data-max=5 data-step=0.1 data-size="xs">
                </button>
                 <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
                                            <p class="calories"><img src="img/icon-budget.png" style="">Rp <span><?php echo $termurah['harga'];?></span>,00</p>
                                            <p style="text-transform: capitalize;" class="calories"><span><?php $r=$termurah['keterangan']; $a=explode(",", $r); echo $a['0']; ?></span></p>
                                                                                <div class="button-wrapper">
                                            <a href="lihatbarang.php?id=<?php echo $termurah['id'];?>" class="button-blue view-menu-item" title="">VIEW</a>
          ...
          <button class="order-customization-link button button-red <?php if($_SESSION['uid']){echo'';}else{echo'hidden';}?>" type="submit">Add to Cart</button>
            ...
            <input type="hidden" name="foto" value="<?php echo $termurah["gambar"]; ?>" />
            <input type="hidden" name="nama" value="<?php echo $termurah["judul"]; ?>" />
            <input type="hidden" name="id" value="<?php echo $termurah["id"]; ?>" />
            <input type="hidden" name="harga" value="<?php echo $termurah["harga"]; ?>" />
              <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
