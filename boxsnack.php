<?php
 ...
 $limit = (isset($_GET['limit'])) ? $_GET['limit'] :4;
  ...
  $inputbox=$_POST['inputbox'];
   ...
   $query ="SELECT * FROM barang where CONCAT(judul,harga,keterangan,jenispaket) like '%".$inputbox."%' AND  jenispaket='boxsnack'";
...
$results=$paginator->getData($limit,$page);
    ...
    $query ="SELECT * FROM barang where jenispaket like '%boxsnack%'";
...
<?php include'header.php';?>
   <div id="bodyholder">
        ...
        <a href="boxsnack.php" class="order-customization-link button button-red <?php if(isset($_POST['inputbox'])){ echo"";}else{echo 'hidden';}?>">Kembali</a>
            <h1 class="item-title blue"><?php if(isset($_POST['inputbox'])){ echo"Pencarian Box Snack";}else{echo'Box Snack';}?></h1>
 ...
 <?php
        ...
        $boxsnack=$results->data[$i]; 
        ?>
                               
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 centered menu-hover">
                                       <a href="lihatbarang.php?id=<?php echo $boxsnack['id'];?>" class="active" >
                                        <img width="250" height="250" class="img-circle" src="<?php echo "imgevent/".$boxsnack['gambar'];?>" class="items" alt="Gambar">
                                           </a>
                                        <h2 class="item-name"><?php echo $boxsnack['judul'];?></h2>
                                        <?php if($_SESSION['uid']){echo'<form method="post" action="?action=rating">';}else{echo"";}?>
                  <input type="hidden" name="idrating" value="<?php echo $boxsnack['id']; ?>">
                <input type="hidden" name="namarating" value="<?php echo $boxsnack['judul']; ?>">
                <input type="hidden" name="gambarrating" value="<?php echo $boxsnack['gambar']; ?>">
                  <button onclick="document.getElementById('login').style.display='<?php if($_SESSION['uid']){echo'none';}else{echo"block";}?>'"> 
                <input  name="ratingdepan" id="input-21e" value="<?php echo $boxsnack["rating"]; ?>"   class="rating" data-min=0.001 data-max=5 data-step=0.001 data-size="xs">
                </button>
                 <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
                                            <p class="calories"><img src="img/icon-budget.png" style="">Rp <span><?php echo $boxsnack['harga'];?></span>,00</p>
                                            <p style="text-transform: capitalize;" class="calories"><span><?php $r=$boxsnack['keterangan']; $a=explode(",", $r); echo $a['0']; ?></span></p>
                                                                                <div class="button-wrapper">
                                            <a href="lihatbarang.php?id=<?php echo $boxsnack['id'];?>" class="button-blue view-menu-item" title="">VIEW</a>
          ...
          <button class="order-customization-link button button-red <?php if($_SESSION['uid']){echo'';}else{echo'hidden';}?>" type="submit">Add to Cart</button>
            ...
            <input type="hidden" name="foto" value="<?php echo $boxsnack["gambar"]; ?>" />
            <input type="hidden" name="nama" value="<?php echo $boxsnack["judul"]; ?>" />
            <input type="hidden" name="id" value="<?php echo $boxsnack["id"]; ?>" />
            <input type="hidden" name="harga" value="<?php echo $boxsnack["harga"]; ?>" />
              <?php if($_SESSION['uid']){echo'</form >';}else{echo"";}?>
